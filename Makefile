...
setup:
	...
	# MENGGUNAKAN USER ROOT AGAR BISA GRANT PRIVILEGES
	$(SAIL) exec mysql mysql -u root -p$(DB_PASSWORD) -e "CREATE DATABASE IF NOT EXISTS \`$(DB_NAME)\`; GRANT ALL PRIVILEGES ON \`$(DB_NAME)\`.* TO 'sail'@'%';"
	@echo "✅ Database '$(DB_NAME)' siap."

	@echo "6. Migrasi Database..."
	$(SAIL) artisan migrate --seed

	@echo "7. Menjalankan Storage Link..."
	$(SAIL) artisan storage:link

	@echo "8. Setup Frontend (NPM Install & Build)..."
	$(SAIL) npm install
	$(SAIL) npm run build
...
db-create:
	...
	$(SAIL) exec mysql mysql -u root -p'$(DB_PASSWORD)' -e "CREATE DATABASE IF NOT EXISTS \`$(name)\`; GRANT ALL PRIVILEGES ON \`$(name)\`.* TO 'sail'@'%';"

db-drop:
	@echo "Menghapus database: $(name)..."
	$(SAIL) exec mysql mysql -u root -p'$(DB_PASSWORD)' -e "DROP DATABASE IF EXISTS \`$(name)\`;"
