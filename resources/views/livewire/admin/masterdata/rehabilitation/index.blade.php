<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
                <h2 class="text-lg font-bold text-slate-800 leading-tight">Rehabilitation Program</h2>
                <p class="text-xs text-slate-500">Manage therapy phases and videos.</p>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <a href="{{ route('admin.rehabilitation.create') }}"
        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th scope="col"
                        class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Program
                        Name</th>
                    <th scope="col"
                        class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Phase</th>
                    <th scope="col"
                        class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                        Description</th>
                    <th scope="col"
                        class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Video
                        Guide</th>
                    <th scope="col"
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                ...
                                {{ $item->rehabType?->name ?? 'Unassigned' }}
                            </span>
                        </td>

                        <td class="px-6 py-4">
                            <div class="text-sm text-slate-600 max-w-xs truncate" title="{{ $item->description }}">
                                {{ $item->description }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($item->video_url)
                                <a href="{{ $item->video_url }}" target="_blank"
                                    class="inline-flex items-center gap-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg border border-red-100 transition-colors">
                                    <i class="fab fa-youtube text-sm"></i> Watch
                                </a>
                            @else
                                <span class="text-xs text-slate-400 italic">No video</span>
                            @endif
                        </td>

                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.rehabilitation.edit', $item->id) }}"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center text-slate-400">
                                ...
                                <h3 class="text-slate-900 font-bold text-lg">No Programs Found</h3>
                                <p class="text-sm mt-1">Try adjusting your search or add a new rehabilitation program.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div
        ...
        <span>Showing {{ count($data) }} programs</span>
    </div>
</div>