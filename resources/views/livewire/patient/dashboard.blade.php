<div class="space-y-6">
    ...
    <div class="relative w-full rounded-3xl overflow-hidden bg-white shadow-xl shadow-teal-900/5 border border-slate-100 group">
    
    <div class="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-teal-50 to-transparent opacity-60 transition-opacity group-hover:opacity-80"></div>
    <img src="{{ asset('assets/images/hero-img.png') }}" class="absolute bottom-0 right-0 h-full w-auto object-cover opacity-20 mix-blend-multiply translate-y-4 translate-x-4 md:translate-x-0 transition-transform duration-700 group-hover:scale-105" alt="Rehabilitation Hero">

    <div class="relative z-10 p-8 md:p-10">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div class="max-w-xl">
                
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-100/80 border border-teal-200 text-teal-700 text-[11px] font-bold uppercase tracking-wider mb-4">
                ...
                @else
                    ...
                    @else
                        <p class="text-slate-500 text-lg leading-relaxed mb-8 max-w-lg">
                            You don't have any rehabilitation program yet. Request a schedule for meeting a doctor to start your journey.
                        </p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/40 flex flex-col justify-between">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-8">
                    ...
                    <div class="relative w-32 h-32 shrink-0">
                        <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                            ...
                            <path class="text-teal-500 drop-shadow-md transition-all duration-1000 ease-out" stroke-dasharray="{{ $progressPercentage }}, 100" stroke-width="3" stroke-linecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                        </svg>
                    ...
                    <div class="grid grid-cols-2 gap-4 w-full">
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <div class="text-2xl font-bold text-slate-800">{{ $completedDays }} <span class="text-sm text-slate-400 font-normal">days</span></div>
                        </div>
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <div class="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-1">Left</div>
                            <div class="text-2xl font-bold text-slate-800">{{ $remainingDays }} <span class="text-sm text-slate-400 font-normal">days</span></div>
                        </div>
                    </div>
                </div>
            @else
        ...
        <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden">
            ...
            @if($checkConsultation)
                @if(is_null($checkConsultation->doctor_id))
                    ...
                    <div class="flex flex-col items-center justify-center py-6 text-center h-full">
                        <div class="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mb-4 animate-pulse text-amber-500">
                            <i class="fas fa-clock text-2xl"></i>
                        </div>
                        <h5 class="text-lg font-bold text-slate-800">Waiting for Confirmation</h5>
                        <p class="text-slate-500 text-sm mt-2 max-w-xs">We have received your request. We are currently matching you with an available specialist.</p>
                    </div>
                @else
...
<div class="space-y-4">
    ...
    <!-- Date & Time Card -->
    <div class="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-white">
        <div class="w-14 h-14 rounded-lg bg-slate-100 flex flex-col items-center justify-center">
        ...
        <div class="relative">
            <div class="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden">
                @if($checkConsultation->doctor?->profile_photo_path)
                    <img
                        src="{{ Storage::url($checkConsultation->doctor->profile_photo_path) }}"
        ...
        <div class="min-w-0">
            ...
            <p class="text-xs text-slate-400 truncate">
                ...
                @endif
            @else
                <!-- Empty State / Request Call to Action -->
                <div class="flex flex-col h-full">
                    <div class="bg-gradient-to-br from-slate-50 to-white rounded-2xl border border-slate-200 p-5 flex flex-col items-center justify-center text-center flex-grow">
                        <p class="text-slate-600 font-medium mb-1">Time for a checkup?</p>
                        <p class="text-xs text-slate-400 mb-6 px-4">Schedule a session with our specialists to track your recovery progress.</p>
                        
                        <button onclick="requestConsultation()" 
                                class="w-full group relative flex items-center justify-center gap-2 px-6 py-3 bg-white text-slate-900 border-2 border-slate-100 rounded-xl font-bold transition-all hover:border-teal-500 hover:text-teal-600 hover:shadow-md cursor-pointer">
                            <span>Request Schedule</span>
                            <i class="fas fa-calendar-plus transition-transform group-hover:scale-110"></i>
                            <span class="absolute top-0 right-0 -mt-1 -mr-1 flex h-3 w-3">
                                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span>
    ...
    <div x-data="{ showModal: false, selectedItem: null }" class="col-span-1 lg:col-span-2 mt-8">
        <div class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                 ...
                 <div class="hidden md:flex w-12 h-12 rounded-full bg-indigo-50 items-center justify-center text-indigo-600">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors duration-200 group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span class="font-bold text-slate-700">{{ \Carbon\Carbon::parse($item->date)->format('d M Y') }}</span>
                                    <span class="text-xs text-slate-400 font-medium">{{ \Carbon\Carbon::parse($item->time)->format('H:i') }} WIB</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden text-slate-400 text-xs shadow-sm border border-slate-200">
                                         @if($item->doctor && $item->doctor->profile_photo_path)
                                            <img src="{{ Storage::url($item->doctor->profile_photo_path) }}" class="w-full h-full object-cover">
                                        @else
                                            <i class="fas fa-user-md text-lg"></i>
                                        @endif
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-700">{{ $item->doctor->name ?? 'Waiting Assignment' }}</span>
                                        <span class="text-xs text-slate-400">{{ $item->location ?? 'Online' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                             ...
                             <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold border {{ $colorClass }}">
                                    {{ ucfirst($item->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <button @click="selectedItem = {{ json_encode([
                                    ...
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-teal-600 hover:border-teal-400 hover:bg-teal-50 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5" title="View Details">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-slate-400 text-sm mt-1">You haven't had any consultations yet.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
                ...
                <div x-show="showModal"
                     ...
                     <div class="bg-gradient-to-br from-slate-900 to-slate-800 px-6 py-5 relative overflow-hidden">
                        ...
                        <div class="flex items-center justify-between relative z-10">
                            ...
                            <button @click="showModal = false" class="text-white/60 hover:text-white hover:bg-white/10 rounded-full p-2 transition-colors">
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-dashed border-slate-200">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Date & Time</p>
                                            <p class="font-bold text-slate-800 text-lg">
                                                <span x-text="new Date(selectedItem.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })"></span>
                                                <span class="text-slate-300 mx-1">|</span>
                                                <span class="text-base font-medium" x-text="selectedItem.time.substring(0,5)"></span>
                                            </p>
                                        </div>
                                    </div>
                                    
                                    <div class="text-left sm:text-right">
                                         <span class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wide"
                                              :class="{
                                ...
                                <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-4">
                                     <div class="w-12 h-12 rounded-full bg-white flex items-center justify-center text-slate-400 shadow-sm border border-slate-200">
                                        <i class="fas fa-user-md text-xl"></i>
                                     </div>
                                     <div class="flex-1">
                                         <p class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5">Doctor</p>
                                         <p class="font-bold text-slate-800" x-text="selectedItem.doctor_name"></p>
                                     </div>
                                     <div class="text-right">
                                         <p class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5">Location</p>
                                         <p class="font-bold text-slate-800 text-sm" x-text="selectedItem.location || 'Online'"></p>
                                     </div>
                                </div>
                                ...
                                <div x-show="selectedItem.description">
                                    ...
                                    <p class="text-slate-600 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100" x-text="selectedItem.description"></p>
                                </div>
                            </div>
                        </template>
                     </div>
