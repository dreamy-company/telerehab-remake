<div x-data="{ mobileMenuOpen: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="py-1">
                            <button @click="switchLang('EN')"
                                class="w-full text-left px-4 py-3 text-sm font-semibold text-slate-600 hover:bg-[#17B8A6]/5 hover:text-[#17B8A6] flex items-center gap-3 transition-colors">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @if (Auth::user())
                        <a href="{{ route('dashboard') }}"
                            class="w-full py-3 px-4 rounded-xl border-2 border-slate-200 text-slate-600 font-bold flex justify-center hover:border-[#17B8A6] hover:text-[#17B8A6]">Dashboard</a>

                    @else
                        <a href="{{ route('auth.login') }}"
                            class="px-8 py-3 rounded-xl border-2 border-slate-200 text-slate-600 font-bold hover:border-[#17B8A6] hover:text-[#17B8A6] transition-all duration-300 whitespace-nowrap">Sign
                            In</a>
                        <a href="{{ route('auth.register') }}"
                            class="px-8 py-3 rounded-xl bg-[#17B8A6] text-white font-bold shadow-lg shadow-[#17B8A6]/20 hover:bg-[#14a393] transition-all duration-300 whitespace-nowrap">Create
                            Account</a>
                    @endif
                </div>
            </div>
    ...
    <div class="relative z-50" aria-labelledby="slide-over-title" role="dialog" aria-modal="true"
        ...
        <div x-show="mobileMenuOpen" x-transition:enter="ease-in-out duration-300" x-transition:enter-start="opacity-0"
            ...
            class="fixed inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity" @click="mobileMenuOpen = false">
        </div>
                    ...
                    <div x-show="mobileMenuOpen" x-transition:enter="transform transition ease-in-out duration-300"
                        ...
                        <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-2xl">
                            ...
                            <div class="px-6 py-6 flex flex-col gap-4">
                                ...
                                @else
                                    <a href="{{ route('auth.login') }}"
                                        class="w-full py-3 rounded-xl border-2 border-slate-200 text-slate-600 font-bold flex justify-center hover:border-[#17B8A6] hover:text-[#17B8A6]">Sign
                                        In</a>
                                    <a href="{{ route('auth.register') }}"
                                        class="w-full py-3 rounded-xl bg-[#17B8A6] text-white font-bold flex justify-center shadow-lg shadow-[#17B8A6]/20">Create
                                        Account</a>
                                @endif
                            </div>
                        </div>
