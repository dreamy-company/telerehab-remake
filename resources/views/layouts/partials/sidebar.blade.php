    ...
    <nav id="sidebarNav" class="space-y-3 flex-1">
        ...
        @foreach ($menuItems as $item)
            ...
            @if (empty($item['subMenu']))
                ...
                <a href="{{ $item['route'] }}" class="flex items-center gap-3 p-3 rounded-xl hover:bg-teal-50 transition-colors
                       {{ $isActive ? 'bg-teal-100 text-teal-600 font-semibold nav-active' : 'text-gray-600' }}" wire:navigate>
            ...
            @else
                ...
                <div x-data="{ open: {{ $isActive ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-teal-50 transition-colors
                                {{ $isActive ? 'bg-teal-100 text-teal-600 font-semibold' : 'text-gray-600' }}">
                        <i class="fas {{ $item['icon'] }} w-5"></i>
                        <span>{{ $item['name'] }}</span>
                        <i class="fas fa-chevron-down w-4 ml-auto transition-transform" :class="open ? 'rotate-180' : ''"></i>
                    </button>
                    ...
                    <div x-show="open" x-cloak class="pl-6 mt-2 space-y-2">
                        @foreach ($item['subMenu'] as $subItem)
                            ...
                            <a href="{{ $subItem['route'] }}" class="flex items-center gap-3 p-2 rounded-lg hover:bg-teal-50 transition-colors
